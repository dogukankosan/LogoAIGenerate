...
namespace ReplicateImageAI.Bussines
{
   internal class SQLSettingsValidator
    {
        internal static bool ValidateSettings(TextEdit serverName, TextEdit port, TextEdit databaseName, TextEdit username, TextEdit password, TextEdit companyNo, TextEdit periodNo,ComboBoxEdit erpType)
        {
            #region Server Name
            ...
            #region ERP Name
            if (string.IsNullOrWhiteSpace(erpType.Text))
            {
                XtraMessageBox.Show("ERP türü boş geçilemez", "Hatalı Giriş", MessageBoxButtons.OK, MessageBoxIcon.Error);
                erpType.Focus();
                return false;
            }
            #endregion
